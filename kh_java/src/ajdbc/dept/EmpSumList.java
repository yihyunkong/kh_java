...
public class EmpSumList extends JFrame implements ActionListener {
	...
	// DB 연동
	Connection con 			= null; // 물리적으로 떨어져있는 오라클 서버에 연결통로 확보
	PreparedStatement ps 	= null; // 개발자가 작성한 select문을 전달하고 오라클 서버에 처리 요청
	ResultSet rs 			= null; // 오라클 서버에서 조회한 결과를 반환해주면 커서 조작하기
	
	DBConnectionMgr dbMgr = new DBConnectionMgr();
	
	public EmpSumList(){
		jbtn_select.addActionListener(this); // 액션 퍼폼드가 내 안에 있다.
		initDisplay();
	}
